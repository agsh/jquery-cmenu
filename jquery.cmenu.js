...
(function ($, window) {
	...
	function create_cmenu_object(actions) {
		...
		//menuOuter.append(cmenu_object.jq);
	...
	function render_item(menu, i, radio) { /* {{{ */
		...
		if (radio && caption === radio) { // radio
			caption = '<strong>' + a.caption + '</strong>';
		} else { // other

		}

		return '<div class="cmenu-item" item_id="' + i + '" ' +
			(a.disabled ?
				// Недоступный элемент
				'style="color:#808080;" ' :
				// Доступный элемент
			...
			'><span class="cmenu-icon">' +
			(a.icon ? image(a.icon, ' ') : '') +
			'</span><span class="cmenu-title"> ' + caption +
			'</span><span class="sprite-icon ' + (a.submenu ? 'cmenu-has-submenu' : '') + '"></span>' +
			'</div>';
		/* }}} */
	}
	...
	function render_menu(menu) {            /* Render menu items    {{{ */
		...
		var h = '<div class="cmenu-left-line"></div>';
		for (var i in menu.a) {
			if (menu.a[i]) {
	...
	$.cmenu = {
		...
		show: function (menu, parentNode, position) {           /* Show menu near parentNode    {{{ */
			...
			if (position == 'left') {
				...
				if ($(parentNode).hasClass('cmenu-item')) {
					menu.jq.css('left',
	...
	$.fn.bindMenu = function (event, menu, position) {/* jQuery-plugin for menu binding {{{ */
		...
		if (!$('#menuOuter').length) {
			$('body').append('<div id="menuOuter"></div>');
		}
		menuOuter = $('#menuOuter');
		menuOuter.css('position', 'absolute');
